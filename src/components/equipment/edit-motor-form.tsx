...
export function EditMotorForm({ equipment }: { equipment: Equipment }) {
  const [isOpen, setIsOpen] = useState(false);
  const [isSaving, setIsSaving] = useState(false);
  const { firestore } = useFirebase();
  const { toast } = useToast();
  const [model, setModel] = useState(equipment.motorModel || '');
  const handleSave = async () => {
    setIsSaving(true);
    try {
      await updateDoc(doc(firestore, 'equipment', equipment.id), { motorModel: model });
      toast({ title: "Updated" });
      setIsOpen(false);
    } catch (e: any) {
