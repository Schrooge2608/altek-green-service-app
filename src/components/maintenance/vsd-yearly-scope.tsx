...
import { useCollection, useMemoFirebase, useUser, addDocumentNonBlocking, updateDocumentNonBlocking, useFirebase } from '@/firebase';
...
const lifecycleChecklistItems = [
    ...
    { component: 'DC Bus Capacitors', action: 'Visual & ESR test.', metric: 'Look for bulging or leaking electrolyte.', reason: 'DC bus capacitors have a finite life (usually 5–10 years).' },
  ...
  return (
    <div className="max-w-4xl mx-auto p-4 sm:p-8 bg-background">
        <div className="flex justify-end mb-4 gap-2 print:hidden items-center">
            ...
            {isEditMode && !isReadOnly && (
                <Button onClick={handleComplete} disabled={isSaving || !techSignature} className="bg-green-600 hover:bg-green-700">
                    {isSaving ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Save className="mr-2 h-4 w-4" />}
                    Mark as Complete
                </Button>
            )}
        ...
        <Card className="p-8 shadow-lg border-2 border-primary/20 bg-card">
            ...
            <Card className="my-8">
                <CardHeader>
                    <CardTitle>Safety & Task Scans</CardTitle>
                </CardHeader>
            ...
            <Card className="mt-8">
                <CardHeader><CardTitle>Component Life-Cycle Testing</CardTitle></CardHeader>
                <CardContent>
                    <Table>
                        <TableHeader>
                            <TableRow>
                    ...
                    <div className="bg-slate-50 p-4 rounded-lg border border-slate-200">
                        ...
                        {techSignature ? (
                            <div className="border border-slate-200 bg-white p-2 rounded-md text-center">
                                ...
                                {!isReadOnly && <Button type="button" variant="ghost" size="sm" onClick={() => setTechSignature("")} className="text-red-500 mt-2 text-xs">Clear Signature</Button>}
                            </div>
                        ) : (
