...
export function DivisionPerformanceDashboard({ plantName }: DivisionPerformanceDashboardProps) {
    ...
    const divisionSummary = useMemo(() => {
        ...
        equipment.forEach(eq => {
            ...
            const currentUptime = Math.min(100, Math.max(0, uptimePercent));
    ...
    const avgUptime = useMemo(() => {
        if (!equipment || equipment.length === 0 || !isMounted) return 0;
        const now = new Date();
        const daysInMonth = new Date(now.getFullYear(), now.getMonth() + 1, 0).getDate();
        const totalHoursInMonth = daysInMonth * 24;
    ...
    const totalPowerConsumption = useMemo(() => {
        if (!equipment || !isMounted) return 0;
        const now = new Date();
        const daysInMonth = new Date(now.getFullYear(), now.getMonth() + 1, 0).getDate();
        const totalHoursInMonth = daysInMonth * 24;
    ...
    return (
        <Card>
            ...
            <CardContent className="grid gap-8">
                <div className="grid gap-4 grid-cols-2 md:grid-cols-5">
                    <Card>
                        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                            <CardTitle className="text-[10px] uppercase font-bold text-muted-foreground">VSDs</CardTitle>
                            <Cpu className="h-4 w-4 text-muted-foreground" />
                        </CardHeader>
                        <CardContent>
                            {isLoading ? <Skeleton className="h-6 w-12" /> : <div className="text-xl font-bold">{totalVSDs}</div>}
                        </CardContent>
                    </Card>
                    <Card>
                        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                            <CardTitle className="text-[10px] uppercase font-bold text-muted-foreground">Protect</CardTitle>
                            <Shield className="h-4 w-4 text-muted-foreground" />
                        </CardHeader>
                        <CardContent>
                            {isLoading ? <Skeleton className="h-6 w-12" /> : <div className="text-xl font-bold">{totalProtection}</div>}
                        </CardContent>
                    </Card>
                    <Card>
                        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                            <CardTitle className="text-[10px] uppercase font-bold text-muted-foreground">UPS</CardTitle>
                            <Power className="h-4 w-4 text-muted-foreground" />
                        </CardHeader>
                        <CardContent>
                            {isLoading ? <Skeleton className="h-6 w-12" /> : <div className="text-xl font-bold">{totalUps}</div>}
                        </CardContent>
                    </Card>
                    <Card>
                        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                            <CardTitle className="text-[10px] uppercase font-bold text-muted-foreground">Uptime</CardTitle>
                            <Activity className="h-4 w-4 text-muted-foreground" />
                        </CardHeader>
                        <CardContent>
                            {isLoading || !isMounted ? <Skeleton className="h-6 w-20" /> : <div className="text-xl font-bold">{avgUptime.toFixed(1)}%</div>}
                        </CardContent>
                    </Card>
                    <Card>
                        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                            <CardTitle className="text-[10px] uppercase font-bold text-muted-foreground">Energy</CardTitle>
                            <Zap className="h-4 w-4 text-muted-foreground" />
                        </CardHeader>
                        <CardContent>
                            {isLoading || !isMounted ? <Skeleton className="h-6 w-24" /> : <div className="text-xl font-bold">{(totalPowerConsumption / 1000).toFixed(1)} <span className="text-[10px] font-normal">MWh</span></div>}
                        </CardContent>
                    </Card>
                </div>
                ...
                <Card>
                    <CardHeader>
                        <CardTitle className="text-base font-bold uppercase tracking-tight">Average Uptime by Division</CardTitle>
                    </CardHeader>
                    <CardContent>
                        {isLoading || !isMounted ? <Skeleton className="h-[350px] w-full" /> : (
