...
export default function MPESurgeBinMCCPage() {
  const firestore = useFirestore();
  const { toast } = useToast();
  const [isSeeding, setIsSeeding] = useState(false);

  const equipmentQuery = useMemoFirebase(
    () => query(collection(firestore, 'equipment'), where('mcc', '==', 'MPE Surge Bin MCC')),
    [firestore]
  );
  ...
  const handleSeedData = async () => {
    ...
    const items = [
      { name: "Primary Feed Pump E1", model: "ACS600", assigned: "Ntokozo" },
      { name: "Primary Feed Pump E2", model: "ACS600", assigned: "Ntokozo" },
      { name: "Fluidization Pump No 1 (Dry Mining feed)", model: "ACS600", assigned: "Ntokozo" },
      { name: "Fluidization Pump No 2", model: "ACS600", assigned: "Ntokozo" }
    ];
    ...
    try {
      ...
      toast({ title: "Success", description: "MPE Surge Bin MCC equipment list has been populated." });
    } catch (error) {
      console.error(error);
    ...
    <div className="flex flex-col gap-8 p-4 md:p-8 max-w-7xl mx-auto">
      <header className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <Link href={`/equipment/new?mcc=MPE Surge Bin MCC&plant=Mining&division=Surge Bin&location=MPE`}>
