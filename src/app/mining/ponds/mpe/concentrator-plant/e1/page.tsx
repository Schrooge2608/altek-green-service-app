...
export default function MPEConsE1DriveRoomPage() {
  const firestore = useFirestore();
  const { toast } = useToast();
  const [isSeeding, setIsSeeding] = useState(false);

  const equipmentQuery = useMemoFirebase(
    () => query(collection(firestore, 'equipment'), where('mcc', '==', 'MPE Cons E1 Drive Room')),
    [firestore]
  );
  ...
  const handleSeedData = async () => {
    ...
    const items: any[] = [
      { name: "Cleaner Feed Pump E1", manufacturer: "ABB", model: "ACS600", assigned: "Ntokozo" },
      { name: "Cons Pump E1-1", manufacturer: "ABB", model: "ACS600", assigned: "Ntokozo" },
      { name: "Cons Pump E1-2", manufacturer: "ABB", model: "ACS600", assigned: "Ntokozo" },
      { name: "Cleaner Tails Pump E1", manufacturer: "ABB", model: "ACS600", assigned: "Ntokozo" },
      { name: "Scavenger Pump E1", manufacturer: "ABB", model: "ACS600", assigned: "Ntokozo" },
      { name: "Magnetite Pump E1", manufacturer: "ABB", model: "ACS600", assigned: "Ntokozo" },
      { name: "Tails Cyclone E1 Pu 37", manufacturer: "ABB", model: "ACS600", assigned: "Ntokozo" },
      { name: "Tails Densifier Pump E1", manufacturer: "ABB", model: "ACS600", assigned: "Ntokozo" },
      { name: "Tails Pump 1-1", manufacturer: "ABB", model: "ACS600", assigned: "Ntokozo" },
      { name: "Tails Pump 1-2", manufacturer: "ABB", model: "ACS600", assigned: "Ntokozo" },
      { name: "Tails Pump 2-1", manufacturer: "ABB", model: "ACS600", assigned: "Ntokozo" },
      { name: "Tails Pump 2-2", manufacturer: "ABB", model: "ACS600", assigned: "Ntokozo" },
      { name: "Cons Stacker Return Water BA", manufacturer: "ABB", model: "ACS800", assigned: "Ntokozo" }
    ];
    ...
    try {
      ...
      await setDoc(doc(collection(firestore, 'equipment'), upsId), {
        ...
        mcc: 'MPD Cons Boosters MCC',
        upsType: 'UPS',
        status: 'active',
        assignedToName: 'Ntokozo'
      });

      toast({ title: "Success", description: "MPE Cons E1 Drive Room equipment list has been populated." });
    } catch (error) {
      console.error(error);
    ...
    <div className="flex flex-col gap-8 p-4 md:p-8 max-w-7xl mx-auto">
      <header className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-black tracking-tight text-slate-900 uppercase">MPE CONS E1 DRIVE ROOM</h1>
            <p className="text-muted-foreground font-medium uppercase text-[10px] tracking-[0.2em] mt-1 flex items-center gap-2">
              <MapPin className="h-3 w-3 text-primary" />
              MPE • Concentrator Plant • E1 Drive Room
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <Link href={`/equipment/new?mcc=MPE Cons E1 Drive Room&plant=Mining&division=Concentrator Plant&location=MPE`}>
