...
export default function MPEDredgeMCCPage() {
  ...
  const equipmentQuery = useMemoFirebase(
    () => query(collection(firestore, 'equipment'), where('mcc', '==', 'MPE Dredge 1 MCC')),
    [firestore]
  );
  ...
  const handleSeedData = async () => {
    ...
    const items = [
      { name: "Dredge 1 Main Pump No. 1", model: "ACS600", assigned: "Ntokozo" },
      { name: "Dredge 1 Main Pump No. 2", model: "ACS600", assigned: "Ntokozo" },
      { name: "Dredge 1 Ladder Winch", model: "ACS600", assigned: "Ntokozo" },
      { name: "Dredge 1 Swing Winch Right", model: "ACS600", assigned: "Ntokozo" },
      { name: "Dredge 1 Swing Winch Left", model: "ACS600", assigned: "Ntokozo" },
      { name: "Dredge 1 Spud Carriage Motor", model: "ACS600", assigned: "Ntokozo" }
    ];
    ...
    try {
      for (const item of items) {
        ...
        await setDoc(doc(collection(firestore, 'equipment'), eqId), {
          ...
          plant: 'Mining',
          division: 'Dredgers',
          mcc: 'MPE Dredge 1 MCC', // New hierarchical field
          vsdId: vsdId,
      ...
      await setDoc(doc(collection(firestore, 'equipment'), upsId), {
        id: upsId,
        name: 'MPE Dredge 1 UPS 1',
        location: 'MPE',
        plant: 'Mining',
        division: 'Dredgers',
        mcc: 'MPE Dredge 1 MCC',
    ...
    <div className="flex flex-col gap-8 p-4 md:p-8 max-w-7xl mx-auto">
      <header className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          <Link href="/mining/ponds/mpe/dredge-1">
            <Button variant="ghost" size="icon" className="rounded-full hover:bg-slate-200">
              <ArrowLeft className="h-5 w-5" />
            </Button>
          </Link>
          <div>
            <h1 className="text-3xl font-black tracking-tight text-slate-900 uppercase">MPE Dredge 1 MCC</h1>
        ...
        <div className="flex items-center gap-3">
          ...
          <Link href={`/equipment/new?mcc=MPE Dredge 1 MCC&plant=Mining&division=Dredgers&location=MPE`}>
