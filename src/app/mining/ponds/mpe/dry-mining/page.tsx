...
export default function MPEDryMiningPage() {
  ...
  const equipmentQuery = useMemoFirebase(
    () => query(collection(firestore, 'equipment'), where('mcc', '==', 'MPE Dry Mining')),
    [firestore]
  );
  ...
  const handleSeedData = async () => {
    ...
    const items = [
      { name: "Water Booster Pump", type: "VSD", manufacturer: "Siemens", model: "Sinamics G150", mcc: "MPE Dry Mining", assignedToName: "Ntokozo" },
      { name: "Slurry Pump", type: "VSD", manufacturer: "Siemens", model: "Sinamics G150", mcc: "MPE Dry Mining", assignedToName: "Ntokozo" },
    ];
    ...
    try {
      ...
      toast({ title: "Success", description: "MPE Dry Mining equipment list has been populated." });
    } catch (error) {
      console.error(error);
    ...
    <div className="flex flex-col gap-8 p-4 md:p-8 max-w-7xl mx-auto">
      <header className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          <Link href="/mining/ponds/mpe">
            <Button variant="ghost" size="icon" className="rounded-full hover:bg-slate-200">
              <ArrowLeft className="h-5 w-5" />
            </Button>
          </Link>
          <div>
            <h1 className="text-3xl font-black tracking-tight text-slate-900 uppercase">MPE Dry Mining</h1>
        ...
        <div className="flex items-center gap-3">
          ...
          <Link href={`/equipment/new?plant=Mining&division=MPE Dry Mining&mcc=MPE Dry Mining&location=MPE`}>
      ...
      <Card className="border-slate-200 shadow-sm">
        <CardHeader className="bg-slate-50 border-b border-slate-100 pb-4">
          ...
          <CardDescription className="text-xs text-slate-500">
            Manage all variable speed drives, motors, and protection systems for MPE Dry Mining.
          </CardDescription>
        </CardHeader>
