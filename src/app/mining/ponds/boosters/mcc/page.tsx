...
export default function BoostersMCCPage() {
  const firestore = useFirestore();
  const { toast } = useToast();
  const [isSeeding, setIsSeeding] = useState(false);

  const equipmentQuery = useMemoFirebase(
    () => query(collection(firestore, 'equipment'), where('mcc', '==', 'Return Water Boosters MCC')),
    [firestore]
  );
  ...
  const handleSeedData = async () => {
    ...
    const items = [
      { name: "Booster Pump No.1", model: "ABB ACS880", manufacturer: "ABB", assigned: "Ntokozo" },
      { name: "Booster Pump No.2", model: "ABB ACS880", manufacturer: "ABB", assigned: "Ntokozo" }
    ];
    ...
    try {
      for (const item of items) {
        ...
        await setDoc(doc(collection(firestore, 'equipment'), eqId), {
          ...
          location: 'Boosters',
          plant: 'Mining',
          division: 'Return Water Booster Station',
      ...
      toast({ title: "Success", description: "Return Water Boosters MCC equipment list has been populated." });
    } catch (error) {
      console.error(error);
    ...
    <div className="flex flex-col gap-8 p-4 md:p-8 max-w-7xl mx-auto">
      <header className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <Link href={`/equipment/new?mcc=Return Water Boosters MCC&plant=Mining&division=Return Water Booster Station&location=Boosters`}>
