...
export default function MPDConsBoostersMCCPage() {
  const firestore = useFirestore();
  const { toast } = useToast();
  const [isSeeding, setIsSeeding] = useState(false);

  const equipmentQuery = useMemoFirebase(
    () => query(collection(firestore, 'equipment'), where('mcc', '==', 'MPD Cons Boosters MCC')),
    [firestore]
  );
  ...
  const handleSeedData = async () => {
    ...
    const items: any[] = [
      ...
      { name: "Cons Booster 4 - PUMP 2", manufacturer: "ABB", model: "ABB ACS 800", assigned: "Ntokozo" },
      { name: "Cons Booster 4 - PUMP 3", manufacturer: "ABB", model: "ABB ACS 800", assigned: "Ntokozo" },
      { name: "Cons Booster 4 - PUMP 4", manufacturer: "ABB", model: "ABB ACS 800", assigned: "Ntokozo" }
    ];
    ...
    try {
      ...
      toast({ title: "Success", description: "MPD Cons Boosters MCC equipment list has been populated." });
    } catch (error) {
      console.error(error);
    ...
    <div className="flex flex-col gap-8 p-4 md:p-8 max-w-7xl mx-auto">
      <header className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <Link href={`/equipment/new?mcc=MPD Cons Boosters MCC&plant=Mining&division=Cons Boosters&location=MPD`}>
