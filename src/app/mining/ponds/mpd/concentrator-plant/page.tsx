...
/**
 * @fileOverview Node dashboard for MPD Concentrator Plant.
 * Lists the physical control nodes like the main Concentrator MCC.
 */
export default function MPDConcentratorPlantNodesPage() {
  return (
    <div className="flex flex-col gap-8 p-4 md:p-8 max-w-7xl mx-auto">
      <header className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          <Link href="/mining/ponds/mpd">
            <Button variant="ghost" size="icon" className="rounded-full">
              <ArrowLeft className="h-5 w-5" />
            </Button>
          </Link>
          <div>
            <h1 className="text-3xl font-black tracking-tight text-slate-900 uppercase">MPD: CONCENTRATOR PLANT</h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <Card className="group hover:shadow-xl transition-all duration-300 border-slate-200">
          <CardHeader className="pb-2 space-y-1">
            ...
            <CardTitle className="text-xl font-black text-slate-800 uppercase tracking-tight">MPD Cons MCC</CardTitle>
            <CardDescription className="text-xs font-medium text-slate-500 leading-relaxed">
              Main motor control center and switchgear for the concentrator plant.
            </CardDescription>
          </CardHeader>
          <CardContent className="pt-4">
            <Link href="/mining/ponds/mpd/concentrator-plant/mcc">
      ...
      <footer className="mt-12 pt-8 border-t border-dashed border-slate-200 text-center">
        <p className="text-[10px] text-slate-400 font-bold uppercase tracking-[0.3em]">
          Altek Green (v2) Asset Register • MPD Concentrator Plant Control Hierarchy
        </p>
      </footer>
    </div>
  );
