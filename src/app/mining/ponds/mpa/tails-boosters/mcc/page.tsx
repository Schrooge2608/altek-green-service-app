...
export default function TailsBoostersMCCPage() {
  ...
  const equipmentQuery = useMemoFirebase(
    () => query(collection(firestore, 'equipment'), where('mcc', '==', 'MPA Tails Boosters MCC')),
    [firestore]
  );
  ...
  const handleSeedData = async () => {
    ...
    const items = [
      { name: "TB12", manufacturer: "ABB", model: "Unknown", assigned: "Ntokozo" },
      { name: "TB06", manufacturer: "ABB", model: "Unknown", assigned: "Ntokozo" },
    ];
    ...
    try {
      ...
      toast({ title: "Success", description: "MPA Tails Boosters MCC equipment list has been populated." });
    } catch (error) {
      console.error(error);
    ...
    <div className="flex flex-col gap-8 p-4 md:p-8 max-w-7xl mx-auto">
      <header className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <Link href={`/equipment/new?mcc=MPA Tails Boosters MCC&plant=Mining&division=Tails Boosters&location=MPA`}>
      ...
      <Card className="border-slate-200 shadow-sm">
        <CardHeader className="bg-slate-50 border-b border-slate-100 pb-4">
          ...
          <CardDescription className="text-xs text-slate-500">
            Manage all variable speed drives, motors, and protection systems powered by the Tails Boosters MCC.
          </CardDescription>
        </CardHeader>
