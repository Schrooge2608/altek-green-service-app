...
export default function SurgeBinMCCPage() {
  ...
  const equipmentQuery = useMemoFirebase(
    () => query(collection(firestore, 'equipment'), where('mcc', '==', 'MPA Surge Bin MCC')),
    [firestore]
  );
  ...
  const handleSeedData = async () => {
    ...
    const items = [
      { name: "Primary Feed Pump", model: "ACS800", assigned: "Ntokozo" },
    ];
    ...
    try {
      for (const item of items) {
        ...
        await setDoc(doc(collection(firestore, 'equipment'), eqId), {
          ...
          location: 'MPA',
          plant: 'Mining',
          division: 'Surge Bin', // Following the same pattern
          mcc: 'MPA Surge Bin MCC',
          vsdId: vsdId,
          assignedToName: item.assigned,
      ...
      toast({ title: "Success", description: "MPA Surge Bin MCC equipment list has been populated." });
    } catch (error) {
      console.error(error);
    ...
    <div className="flex flex-col gap-8 p-4 md:p-8 max-w-7xl mx-auto">
      <header className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <Link href={`/equipment/new?mcc=MPA Surge Bin MCC&plant=Mining&division=Surge Bin&location=MPA`}>
      ...
      <Card className="border-slate-200 shadow-sm">
        <CardHeader className="bg-slate-50 border-b border-slate-100 pb-4">
          ...
          <CardDescription className="text-xs text-slate-500">
            Manage all variable speed drives, motors, and protection systems powered by the Surge Bin MCC.
          </CardDescription>
        </CardHeader>
