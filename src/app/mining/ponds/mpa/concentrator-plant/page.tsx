...
/**
 * @fileOverview Node dashboard for MPA Concentrator Plant.
 * Lists the physical control nodes like the main Concentrator MCC.
 */
export default function MPAConcentratorPlantNodesPage() {
  return (
    <div className="flex flex-col gap-8 p-4 md:p-8 max-w-7xl mx-auto">
      <header className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          <Link href="/mining/ponds/mpa">
            <Button variant="ghost" size="icon" className="rounded-full">
              <ArrowLeft className="h-5 w-5" />
            </Button>
          </Link>
          <div>
            <h1 className="text-3xl font-black tracking-tight text-slate-900 uppercase">MPA: CONCENTRATOR PLANT</h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <Card className="group hover:shadow-xl transition-all duration-300 border-slate-200">
          <CardHeader className="pb-2 space-y-1">
            ...
            <CardTitle className="text-xl font-black text-slate-800 uppercase tracking-tight">MPA Cons MCC</CardTitle>
          ...
          <CardContent className="pt-4">
            <Link href="/mining/ponds/mpa/concentrator-plant/mcc">
      ...
      <footer className="mt-12 pt-8 border-t border-dashed border-slate-200 text-center">
        <p className="text-[10px] text-slate-400 font-bold uppercase tracking-[0.3em]">
          Altek Green (v2) Asset Register • MPA Concentrator Plant Control Hierarchy
        </p>
      </footer>
    </div>
  );
