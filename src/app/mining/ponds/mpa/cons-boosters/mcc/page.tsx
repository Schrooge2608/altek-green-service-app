...
export default function ConsBoostersMCCPage() {
  ...
  const equipmentQuery = useMemoFirebase(
    () => query(collection(firestore, 'equipment'), where('mcc', '==', 'MPA Cons Boosters MCC')),
    [firestore]
  );
  ...
  const handleSeedData = async () => {
    ...
    const items = [
      { name: "Cons Booster 6 - PUMP 1", manufacturer: "ABB", model: "ABB ACS 880", assigned: "Ntokozo" },
      { name: "Cons Booster 7 - PUMP 1", manufacturer: "ABB", model: "ACS880", assigned: "Ntokozo" },
      { name: "Cons Booster 7 - PUMP 2", manufacturer: "ABB", model: "ACS880", assigned: "Ntokozo" },
    ];
    ...
    try {
      ...
      toast({ title: "Success", description: "MPA Cons Boosters MCC equipment list has been populated." });
    } catch (error) {
      console.error(error);
    ...
    <div className="flex flex-col gap-8 p-4 md:p-8 max-w-7xl mx-auto">
      <header className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <Link href={`/equipment/new?mcc=MPA Cons Boosters MCC&plant=Mining&division=Boosters&location=MPA`}>
      ...
      <Card className="border-slate-200 shadow-sm">
        <CardHeader className="bg-slate-50 border-b border-slate-100 pb-4">
          ...
          <CardDescription className="text-xs text-slate-500">
            Manage all variable speed drives, motors, and protection systems powered by the Cons Boosters MCC.
          </CardDescription>
        </CardHeader>
