...
export default function MPCDredgeMCCPage() {
  ...
  const equipmentQuery = useMemoFirebase(
    () => query(collection(firestore, 'equipment'), where('mcc', '==', 'MPC Dredger MCC')),
    [firestore]
  );
  ...
  const handleSeedData = async () => {
    ...
    const items: any[] = []; // Waiting for dictated list
    ...
    try {
      for (const item of items) {
        ...
        await setDoc(doc(collection(firestore, 'vsds'), vsdId), {
          ...
          equipmentId: eqId,
          model: item.model || 'ACS800',
          manufacturer: 'ABB',
          installationDate: new Date().toISOString().split('T')[0],
          status: 'active',
          assignedToName: item.assigned || 'Ntokozo'
        });
        ...
        await setDoc(doc(collection(firestore, 'equipment'), eqId), {
          ...
          plant: 'Mining',
          division: 'Dredgers',
          mcc: 'MPC Dredger MCC', // New hierarchical field
          vsdId: vsdId,
          assignedToName: item.assigned || 'Ntokozo',
      ...
      toast({ title: "Success", description: "MPC Dredger MCC equipment list has been populated." });
    } catch (error) {
      console.error(error);
    ...
    <div className="flex flex-col gap-8 p-4 md:p-8 max-w-7xl mx-auto">
      <header className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <Link href={`/equipment/new?mcc=MPC Dredger MCC&plant=Mining&division=Dredgers&location=MPC`}>
      ...
      <Card className="border-slate-200 shadow-sm">
        <CardHeader className="bg-slate-50 border-b border-slate-100 pb-4">
          ...
          <CardDescription className="text-xs text-slate-500">
            Manage all variable speed drives, motors, and protection systems powered by the MPC Dredge MCC.
          </CardDescription>
        </CardHeader>
