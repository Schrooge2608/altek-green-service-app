...
export default function MPCTailsBoostersMCCPage() {
  const firestore = useFirestore();
  const { toast } = useToast();
  const [isSeeding, setIsSeeding] = useState(false);

  const equipmentQuery = useMemoFirebase(
    () => query(collection(firestore, 'equipment'), where('mcc', '==', 'MPC Tails Boosters MCC')),
    [firestore]
  );
  ...
  const handleSeedData = async () => {
    ...
    const items: any[] = [
      { name: "Tails Booster TB05", manufacturer: "Siemens", model: "Sinamics G150", assigned: "Ntokozo" },
      { name: "Tails Booster TB06", manufacturer: "ABB", model: "ABB ACS 800", assigned: "Ntokozo" },
      { name: "Tails Booster TB08", manufacturer: "ABB", model: "ABB ACS 800", assigned: "Ntokozo" },
      { name: "Tails Booster TB22", manufacturer: "ABB", model: "ABB ACS 600", assigned: "Ntokozo" },
      { name: "Tails Booster TB23", manufacturer: "ABB", model: "ABB ACS 600", assigned: "Ntokozo" }
    ];
    ...
    try {
      ...
      toast({ title: "Success", description: "MPC Tails Boosters MCC equipment list has been populated." });
    } catch (error) {
      console.error(error);
    ...
    <div className="flex flex-col gap-8 p-4 md:p-8 max-w-7xl mx-auto">
      <header className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <Link href={`/equipment/new?mcc=MPC Tails Boosters MCC&plant=Mining&division=Tails Boosters&location=MPC`}>
