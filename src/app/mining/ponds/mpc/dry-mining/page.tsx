...
export default function MPCDryMiningPage() {
  ...
  const equipmentQuery = useMemoFirebase(
    () => query(collection(firestore, 'equipment'), where('mcc', '==', 'MPC Dry Mining')),
    [firestore]
  );
  ...
  const handleSeedData = async () => {
    ...
    const items = [
      { name: "Water Booster Pump", type: "VSD", manufacturer: "Siemens", model: "Sinamics G150", mcc: "MPC Dry Mining", assignedToName: "Ntokozo" },
      { name: "Slurry Pump", type: "VSD", manufacturer: "Siemens", model: "Sinamics G150", mcc: "MPC Dry Mining", assignedToName: "Ntokozo" },
    ];
    ...
    try {
      ...
      toast({ title: "Success", description: "MPC Dry Mining equipment list has been populated." });
    } catch (error) {
      console.error(error);
    ...
    <div className="flex flex-col gap-8 p-4 md:p-8 max-w-7xl mx-auto">
      <header className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-black tracking-tight text-slate-900 uppercase">MPC Dry Mining</h1>
        ...
        <div className="flex items-center gap-3">
          ...
          <Link href={`/equipment/new?plant=Mining&division=MPC Dry Mining&mcc=MPC Dry Mining&location=MPC`}>
      ...
      <Card className="border-slate-200 shadow-sm">
        <CardHeader className="bg-slate-50 border-b border-slate-100 pb-4">
          ...
          <CardDescription className="text-xs text-slate-500">
            Manage all variable speed drives, motors, and protection systems for MPC Dry Mining.
          </CardDescription>
        </CardHeader>
