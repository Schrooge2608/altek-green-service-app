...
export default function CreateQuotePage() {
  ...
  // 2. Fetch service rates for autocomplete
  ...
  useEffect(() => {
    const fetchNextNumber = async () => {
      try {
        const q = query(collection(firestore, 'quotations'), orderBy('quoteNumber', 'desc'), limit(1));
        const snapshot = await getDocs(q);
        if (!snapshot.empty) {
          const lastNumStr = snapshot.docs[0].data().quoteNumber || '';
          const baseNumStr = lastNumStr.replace(/BD$/, '');
          const lastNum = parseInt(baseNumStr);
          
          if (!isNaN(lastNum)) {
            setQuoteNum((lastNum + 1).toString() + 'BD');
          } else {
            setQuoteNum('1022040BD');
          }
        } else {
          setQuoteNum('1022040BD');
        }
  ...
  const handleSave = async () => {
    if (!clientAddress) return alert("Please select a client or enter an address");
    setIsSaving(true);
    try {
      ...
      await addDocumentNonBlocking(collection(firestore, 'quotations'), {
  ...
  if (!quoteDate) return <div className="flex h-screen items-center justify-center"><Loader2 className="animate-spin" /></div>;
  ...
  return (
    <div className="max-w-5xl mx-auto p-4 sm:p-8 bg-background">
      ...
      <div className="flex justify-between items-center mb-6 no-print">
        ...
        <Button onClick={handleSave} disabled={isSaving || !clientAddress}>
      ...
      <div className="quote-container-wrapper">
        <div 
          className="bg-white p-10 border shadow-sm print:shadow-none print:border-none min-h-[29.7cm] print:min-h-0 text-slate-900 flex flex-col print:block print:h-auto" 
          ...
          <div className="flex justify-between items-start mb-8 gap-8">
            ...
            <div className="w-1/3 text-right">
              <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-sm">
                ...
                <Input 
                  type="text"
                  value={isLoadingNum ? 'Loading...' : quoteNum}
