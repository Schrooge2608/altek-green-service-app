...
export default function EditUserPage() {
  ...
  const clientsQuery = useMemoFirebase(() => query(collection(firestore, 'clients'), orderBy('companyName', 'asc')), [firestore]);
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
          <Card>
            ...
            <CardContent className="grid gap-6 md:grid-cols-2">
              ...
              <div className="md:col-span-2">
                <FormField control={form.control} name="clientId" render={({ field }) => (
                  <FormItem>
                    <FormLabel>Client Assignment</FormLabel>
                    ...
                    <FormDescription>Assigning a client will automatically link this user to the company's SLA and contract details.</FormDescription>
                    <FormMessage />
                  </FormItem>
                )} />
              </div>
           ...
           <Card>
            <CardHeader>
              ...
              <CardDescription>Manage the user's secure PIN for signing documents.</CardDescription>
            </CardHeader>
            <CardContent className="grid gap-6 md:grid-cols-2">
                 <FormField control={form.control} name="signingPin" render={({ field }) => (
                   <FormItem>
                     <FormLabel>4-Digit Signing PIN</FormLabel>
                     <FormControl>
                       <Input type="password" maxLength={4} placeholder="e.g., 1234" {...field} />
                     </FormControl>
                     <FormDescription>Used for identity verification during sign-off.</FormDescription>
                     <FormMessage />
                   </FormItem>
                 )} />
            </CardContent>
           ...
           <Card>
            ...
            <CardContent className="grid gap-6 md:grid-cols-2">
                 <FormField control={form.control} name="sapNumber" render={({ field }) => (<FormItem><FormLabel>SAP Number</FormLabel><FormControl><Input placeholder="Enter SAP number" {...field} value={field.value ?? ''} /></FormControl><FormMessage /></FormItem>)} />
                 <FormField control={form.control} name="rtbsNumber" render={({ field }) => (<FormItem><FormLabel>RTBS Number</FormLabel><FormControl><Input placeholder="Enter RTBS number" {...field} value={field.value ?? ''} /></FormControl><FormMessage /></FormItem>)} />
                 <div className="md:col-span-2"><FormField control={form.control} name="qualifications" render={({ field }) => (<FormItem><FormLabel>Qualifications</FormLabel><FormControl><Textarea placeholder="List qualifications" {...field} value={field.value ?? ''} /></FormControl><FormMessage /></FormItem>)} /></div>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent className="grid gap-6 md:grid-cols-2">
                 <FormField control={form.control} name="nextOfKinName" render={({ field }) => (<FormItem><FormLabel>Next of Kin Name</FormLabel><FormControl><Input placeholder="e.g., Jane Doe" {...field} value={field.value ?? ''} /></FormControl><FormMessage /></FormItem>)} />
                 <FormField control={form.control} name="nextOfKinPhone" render={({ field }) => (<FormItem><FormLabel>Next of Kin Phone</FormLabel><FormControl><Input placeholder="e.g., +27 98 765 4321" {...field} value={field.value ?? ''} /></FormControl><FormMessage /></FormItem>)} />
            </CardContent>
          </Card>
