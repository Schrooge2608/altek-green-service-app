...
function ImageGallery({ title, images }: { title: string; images?: string[] }) {
    ...
    return (
        <div className="mt-4">
            <h4 className="font-semibold text-muted-foreground uppercase text-[10px] tracking-widest mb-2">{title}</h4>
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4 mt-2">
                {images.map((url, index) => {
                    const isDoc = isPdfUrl(url);
                    const fileName = getFileNameFromUrl(url);

                    return (
                        <a key={index} href={url} target="_blank" rel="noopener noreferrer" className="block">
                            <div className="relative group aspect-square border rounded-md overflow-hidden bg-slate-50 flex items-center justify-center">
                                {isDoc ? (
                                    <div className="flex flex-col items-center p-2 text-center">
                                        <FileText className="h-10 w-10 text-red-500 mb-2" />
                                        <span className="text-[10px] font-black text-slate-600 truncate w-full px-1">{fileName}</span>
                                        <span className="text-[8px] uppercase font-bold text-slate-400 mt-1">View PDF</span>
                                    </div>
                                ) : (
