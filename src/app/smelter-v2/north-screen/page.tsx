...
export default function NorthScreenPage() {
  ...
  const equipmentQuery = useMemoFirebase(
    () => query(collection(firestore, 'equipment'), where('division', '==', 'North Screen')),
    [firestore]
  );
  ...
  const handleSeedData = async () => {
    ...
    const items = [
      { name: "Apron Feeder", type: "VSD", manufacturer: "Siemens", model: "N/A", mcc: "North Screen MCC" },
    ];
    ...
    try {
      ...
      toast({ title: "Success", description: "North Screen equipment list has been populated." });
    } catch (error) {
      console.error(error);
    ...
    <div className="flex flex-col gap-8 p-4 md:p-8 max-w-7xl mx-auto">
      <header className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <Link href={`/equipment/new?plant=Smelter&division=North Screen&mcc=North Screen MCC&location=Smelter`}>
      ...
      <Card className="border-slate-200 shadow-sm">
        <CardHeader className="bg-slate-50 border-b border-slate-100 pb-4">
          ...
          <CardDescription className="text-xs text-slate-500">
            Manage all variable speed drives, motors, and protection systems for the North Screen.
          </CardDescription>
        </CardHeader>
