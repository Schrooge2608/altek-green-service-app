...
export default function FbcMccPage() {
  ...
  const equipmentQuery = useMemoFirebase(
    () => query(collection(firestore, 'equipment'), where('mcc', '==', 'F.B.C. MCC')),
    [firestore]
  );
  ...
  const handleSeedData = async () => {
    ...
    const items = [
      { name: "S30 Vibrator Feeder", type: "VSD", manufacturer: "Siemens", model: "Micromaster 440", mcc: "F.B.C. MCC" },
      { name: "S18B Feed Conveyor", type: "VSD", manufacturer: "Control Techniques", model: "UNISP", mcc: "F.B.C. MCC" },
    ];
    ...
    try {
      ...
      toast({ title: "Success", description: "F.B.C. MCC equipment list has been populated." });
    } catch (error) {
      console.error(error);
    ...
    <div className="flex flex-col gap-8 p-4 md:p-8 max-w-7xl mx-auto">
      <header className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-black tracking-tight text-slate-900 uppercase">F.B.C. MCC</h1>
        ...
        <div className="flex items-center gap-3">
          ...
          <Link href={`/equipment/new?plant=Smelter&division=Slag Plant&mcc=F.B.C. MCC&location=Slag Plant`}>
      ...
      <Card className="border-slate-200 shadow-sm">
        <CardHeader className="bg-slate-50 border-b border-slate-100 pb-4">
          ...
          <CardDescription className="text-xs text-slate-500">
            Manage all variable speed drives, motors, and protection systems for F.B.C. MCC.
          </CardDescription>
        </CardHeader>
