...
export default function PCMPage() {
  ...
  // Query equipment specifically for PCM 1 and PCM 2
  const equipmentQuery = useMemoFirebase(
    () => query(collection(firestore, 'equipment'), where('mcc', 'in', ['PCM 1 MCC', 'PCM 2 MCC'])),
    [firestore]
  );
  ...
  const handleSeedData = async () => {
    ...
    const items = [
      { name: "Casting ladle tilter", type: "VSD", manufacturer: "Siemens", model: "Simoreg DC", mcc: "PCM 1 MCC" },
      { name: "West strand (1)", type: "VSD", manufacturer: "Control Techniques", model: "UNISP", mcc: "PCM 1 MCC" },
      { name: "East strand (1)", type: "VSD", manufacturer: "Control Techniques", model: "UNISP", mcc: "PCM 1 MCC" },
      { name: "East strand (2)", type: "VSD", manufacturer: "Control Techniques", model: "UNISP", mcc: "PCM 2 MCC" },
      { name: "West strand (2)", type: "VSD", manufacturer: "Control Techniques", model: "UNISP", mcc: "PCM 2 MCC" },
      { name: "Mould Wash Spray Pump East", type: "VSD", manufacturer: "Control Techniques", model: "UNISP", mcc: "PCM 2 MCC" },
    ];
    ...
    try {
      ...
      toast({ title: "Success", description: "PCM 1 & 2 equipment list has been populated." });
    } catch (error) {
      console.error(error);
    ...
    <div className="flex flex-col gap-8 p-4 md:p-8 max-w-7xl mx-auto">
      <header className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          <Link href="/smelter-v2/iron-injection">
            <Button variant="ghost" size="icon" className="rounded-full hover:bg-slate-200">
              <ArrowLeft className="h-5 w-5" />
            </Button>
          </Link>
          <div>
            <h1 className="text-3xl font-black tracking-tight text-slate-900 uppercase">PCM</h1>
        ...
        <div className="flex items-center gap-3">
          ...
          <Link href={`/equipment/new?plant=Smelter&division=Iron Injection&mcc=PCM 1 MCC&location=Iron Injection`}>
      ...
      <Card className="border-slate-200 shadow-sm">
        <CardHeader className="bg-slate-50 border-b border-slate-100 pb-4">
          ...
          <CardDescription className="text-xs text-slate-500">
            Manage all variable speed drives, motors, and protection systems for PCM 1 & 2.
          </CardDescription>
        </CardHeader>
