...
export default function LimeShedPage() {
  ...
  const equipmentQuery = useMemoFirebase(
    () => query(collection(firestore, 'equipment'), where('mcc', '==', 'LIME SHED')),
    [firestore]
  );
  ...
  const handleSeedData = async () => {
    ...
    const items = [
      { name: "Lime / Sodium Pump 7A", type: "VSD", manufacturer: "Siliconics", model: "Safari" },
      { name: "Lime / Sodium Pump 7B", type: "VSD", manufacturer: "Siliconics", model: "Safari" },
    ];
    ...
    try {
      ...
      toast({ title: "Success", description: "LIME SHED equipment list has been populated." });
    } catch (error) {
      console.error(error);
    ...
    <div className="flex flex-col gap-8 p-4 md:p-8 max-w-7xl mx-auto">
      <header className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-black tracking-tight text-slate-900 uppercase">LIME SHED</h1>
        ...
        <div className="flex items-center gap-3">
          ...
          <Link href={`/equipment/new?mcc=LIME SHED&plant=Smelter&division=MSP&location=Smelter`}>
      ...
      <Card className="border-slate-200 shadow-sm">
        <CardHeader className="bg-slate-50 border-b border-slate-100 pb-4">
          ...
          <CardDescription className="text-xs text-slate-500">
            Manage all variable speed drives, motors, and protection systems powered by the LIME SHED.
          </CardDescription>
        </CardHeader>
