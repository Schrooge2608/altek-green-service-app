...
export default function NZTPCalcinerMCCPage() {
  ...
  const equipmentQuery = useMemoFirebase(
    () => query(collection(firestore, 'equipment'), where('mcc', '==', 'NZTP CALCINER MCC')),
    [firestore]
  );
  ...
  const handleSeedData = async () => {
    ...
    const items = [
      { name: "Conveyor 1A VSD", type: "VSD", manufacturer: "Siemens", model: "Micromaster 440" },
      { name: "Calciner exhaust Fan 15 softstarter", type: "Soft Starter", manufacturer: "Alan Bradley", model: "SMC-FLEX" },
      { name: "Calciner Blower Fan 13 softstarter", type: "Soft Starter", manufacturer: "Alan Bradley", model: "SMC-FLEX" },
      { name: "Perm Roll Drives", type: "VSD", manufacturer: "Siemens", model: "Micromaster 440" },
    ];
    ...
    try {
      ...
      toast({ title: "Success", description: "NZTP CALCINER MCC equipment list has been populated." });
    } catch (error) {
      console.error(error);
    ...
    <div className="flex flex-col gap-8 p-4 md:p-8 max-w-7xl mx-auto">
      <header className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          <Link href="/smelter-v2/msp/nztp">
            <Button variant="ghost" size="icon" className="rounded-full hover:bg-slate-200">
              <ArrowLeft className="h-5 w-5" />
            </Button>
          </Link>
          <div>
            <h1 className="text-3xl font-black tracking-tight text-slate-900 uppercase">NZTP CALCINER MCC</h1>
        ...
        <div className="flex items-center gap-3">
          ...
          <Link href={`/equipment/new?mcc=NZTP CALCINER MCC&plant=Smelter&division=MSP&location=Smelter`}>
      ...
      <Card className="border-slate-200 shadow-sm">
        <CardHeader className="bg-slate-50 border-b border-slate-100 pb-4">
          ...
          <CardDescription className="text-xs text-slate-500">
            Manage all variable speed drives, motors, and protection systems powered by the NZTP CALCINER MCC.
          </CardDescription>
        </CardHeader>
