...
export default function RWPHPage() {
  ...
  const equipmentQuery = useMemoFirebase(
    () => query(collection(firestore, 'equipment'), where('mcc', '==', 'RWPH MCC')),
    [firestore]
  );
  ...
  const handleSeedData = async () => {
    ...
    const items = [
      { name: "Disposal pump 16 VSD", type: "VSD", manufacturer: "Siemens", model: "Sinamics G150" },
      { name: "Disposal pump 17 VSD", type: "VSD", manufacturer: "Siemens", model: "Sinamics G150" },
    ];
    ...
    try {
      ...
      toast({ title: "Success", description: "RWPH MCC equipment list has been populated." });
    } catch (error) {
      console.error(error);
    ...
    <div className="flex flex-col gap-8 p-4 md:p-8 max-w-7xl mx-auto">
      <header className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-black tracking-tight text-slate-900 uppercase">RWPH MCC</h1>
        ...
        <div className="flex items-center gap-3">
          ...
          <Link href={`/equipment/new?mcc=RWPH MCC&plant=Smelter&division=MSP&location=Smelter`}>
      ...
      <Card className="border-slate-200 shadow-sm">
        <CardHeader className="bg-slate-50 border-b border-slate-100 pb-4">
          ...
          <CardDescription className="text-xs text-slate-500">
            Manage all variable speed drives, motors, and protection systems powered by the RWPH MCC.
          </CardDescription>
        </CardHeader>
