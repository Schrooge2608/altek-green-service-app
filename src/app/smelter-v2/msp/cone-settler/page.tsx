...
export default function ConeSettlerPage() {
  ...
  const equipmentQuery = useMemoFirebase(
    () => query(collection(firestore, 'equipment'), where('mcc', '==', 'CONE SETTLER MCC')),
    [firestore]
  );
  ...
  const handleSeedData = async () => {
    ...
    const items = [
      { name: "Cone Settler Pump", type: "VSD", manufacturer: "Siemens", model: "Micromaster 440" },
    ];
    ...
    try {
      ...
      toast({ title: "Success", description: "Cone Settler MCC equipment list has been populated." });
    } catch (error) {
      console.error(error);
    ...
    <div className="flex flex-col gap-8 p-4 md:p-8 max-w-7xl mx-auto">
      <header className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-black tracking-tight text-slate-900 uppercase">CONE SETTLER MCC</h1>
            <p className="text-muted-foreground font-medium uppercase text-[10px] tracking-[0.2em] mt-1 flex items-center gap-2">
              <MapPin className="h-3 w-3 text-primary" />
              Smelter • MSP • Cone Settler
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <Link href={`/equipment/new?mcc=CONE SETTLER MCC&plant=Smelter&division=MSP&location=Smelter`}>
      ...
      <Card className="border-slate-200 shadow-sm">
        <CardHeader className="bg-slate-50 border-b border-slate-100 pb-4">
          ...
          <CardDescription className="text-xs text-slate-500">
            Manage all variable speed drives, motors, and protection systems powered by the Cone Settler MCC.
          </CardDescription>
        </CardHeader>
