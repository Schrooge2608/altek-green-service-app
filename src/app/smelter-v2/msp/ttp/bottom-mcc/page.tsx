...
export default function BottomMCCPage() {
  ...
  const equipmentQuery = useMemoFirebase(
    () => query(collection(firestore, 'equipment'), where('mcc', '==', 'Bottom MCC')),
    [firestore]
  );
  ...
  const handleSeedData = async () => {
    ...
    const items = [
      // Fan/Pump (ABB)
      { name: "Fan 2", type: "VSD", manufacturer: "ABB", model: "ACS800", isUps: false },
      { name: "Fan 4", type: "VSD", manufacturer: "ABB", model: "ACS800", isUps: false },
      { name: "PUMP 28", type: "VSD", manufacturer: "ABB", model: "ACS800", isUps: false },
      
      // HTR MODULES (Allen Bradley)
      ...Array.from({ length: 8 }, (_, i) => ({
        name: `HTR MODULE ${i + 1}`, type: "VSD", manufacturer: "Allen Bradley", model: "Powerflex 40", isUps: false
      })),

      // MAGNETS (Allen Bradley)
      ...Array.from({ length: 15 }, (_, i) => ({
        name: `MAGNET ${i + 1}`, type: "VSD", manufacturer: "Allen Bradley", model: "Powerflex 40", isUps: false
      })),

      // REM (Siemens)
      ...Array.from({ length: 4 }, (_, i) => ({
        name: `REM${i + 1}`, type: "VSD", manufacturer: "Siemens", model: "Micromaster 440", isUps: false
      }))
    ];
    ...
    try {
      ...
      toast({ title: "Success", description: "Bottom MCC equipment list has been populated." });
    } catch (error) {
      console.error(error);
    ...
    <div className="flex flex-col gap-8 p-4 md:p-8 max-w-7xl mx-auto">
      <header className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <Link href={`/equipment/new?mcc=Bottom MCC&plant=Smelter&division=MSP&location=Smelter`}>
      ...
      <Card className="border-slate-200 shadow-sm">
        <CardHeader className="bg-slate-50 border-b border-slate-100 pb-4">
          ...
          <CardDescription className="text-xs text-slate-500">
            Manage all variable speed drives, motors, and protection systems powered by the Bottom MCC.
          </CardDescription>
        </CardHeader>
