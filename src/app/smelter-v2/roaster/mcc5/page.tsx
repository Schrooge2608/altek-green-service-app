...
export default function RoasterMCC5Page() {
  ...
  const equipmentQuery = useMemoFirebase(
    () => query(collection(firestore, 'equipment'), where('mcc', '==', 'Roaster MCC 5')),
    [firestore]
  );
  ...
  const handleSeedData = async () => {
    ...
    const items = [
      { name: "Elliot Blower 1", type: "VSD", manufacturer: "Siemens", model: "Rubicon Perfect Harmony" },
      { name: "Elliot Blower 2", type: "VSD", manufacturer: "Siemens", model: "Rubicon Perfect Harmony" },
      { name: "Elliot Blower 3", type: "VSD", manufacturer: "Siemens", model: "Rubicon Perfect Harmony" },
    ];
    ...
    try {
      ...
      toast({ title: "Success", description: "Roaster MCC 5 equipment list has been populated." });
    } catch (error) {
      console.error(error);
    ...
    <div className="flex flex-col gap-8 p-4 md:p-8 max-w-7xl mx-auto">
      <header className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-black tracking-tight text-slate-900 uppercase">ROASTER MCC 5</h1>
        ...
        <div className="flex items-center gap-3">
          ...
          <Link href={`/equipment/new?mcc=Roaster MCC 5&plant=Smelter&division=Roaster&location=Smelter`}>
      ...
      <Card className="border-slate-200 shadow-sm">
        <CardHeader className="bg-slate-50 border-b border-slate-100 pb-4">
          ...
          <CardDescription className="text-xs text-slate-500">
            Manage all variable speed drives, motors, and protection systems powered by Roaster MCC 5.
          </CardDescription>
        </CardHeader>
