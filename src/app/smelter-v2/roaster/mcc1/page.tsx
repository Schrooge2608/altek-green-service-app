...
export default function RoasterMCC1Page() {
  ...
  const equipmentQuery = useMemoFirebase(
    () => query(collection(firestore, 'equipment'), where('mcc', '==', 'Roaster MCC 1')),
    [firestore]
  );
  ...
  const handleSeedData = async () => {
    ...
    const items = [
      { name: "C5 Conveyor to R1", type: "VSD", manufacturer: "Control Techniques", model: "UNI SP" },
      { name: "C4 Conveyor R2", type: "VSD", manufacturer: "UNI-SP", model: "UNI SP" },
      { name: "Holmes Blower 2", type: "VSD", manufacturer: "ABB", model: "ACS800" },
      { name: "UPS A", type: "UPS", manufacturer: "Riello HPS, MPM, MPS", model: "10Kva to 80Kva" },
      { name: "UPS B", type: "UPS", manufacturer: "Riello HPS, MPM, MPS", model: "10Kva to 80Kva" },
    ];
    ...
    try {
      ...
      toast({ title: "Success", description: "Roaster MCC 1 equipment list has been populated." });
    } catch (error) {
      console.error(error);
    ...
    <div className="flex flex-col gap-8 p-4 md:p-8 max-w-7xl mx-auto">
      <header className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-black tracking-tight text-slate-900 uppercase">ROASTER MCC 1</h1>
        ...
        <div className="flex items-center gap-3">
          ...
          <Link href={`/equipment/new?mcc=Roaster MCC 1&plant=Smelter&division=Roaster&location=Smelter`}>
      ...
      <Card className="border-slate-200 shadow-sm">
        <CardHeader className="bg-slate-50 border-b border-slate-100 pb-4">
          ...
          <CardDescription className="text-xs text-slate-500">
            Manage all variable speed drives, motors, and protection systems powered by Roaster MCC 1.
          </CardDescription>
        </CardHeader>
