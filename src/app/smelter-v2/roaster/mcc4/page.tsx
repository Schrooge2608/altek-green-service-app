...
export default function RoasterMCC4Page() {
  ...
  const equipmentQuery = useMemoFirebase(
    () => query(collection(firestore, 'equipment'), where('mcc', '==', 'Roaster MCC 4')),
    [firestore]
  );
  ...
  const handleSeedData = async () => {
    ...
    const items = [
      { name: "Scrubber Fan 2", type: "VSD", manufacturer: "Siemens", model: "Sinamics G150" },
      { name: "Chip Conveyor", type: "VSD", manufacturer: "Control Techniques", model: "UNI SP" },
      { name: "Scrubber Fan 1", type: "VSD", manufacturer: "ABB", model: "ACS880" },
      { name: "Aerzen Blower", type: "VSD", manufacturer: "ABB", model: "ACS800" },
    ];
    ...
    try {
      ...
      toast({ title: "Success", description: "Roaster MCC 4 equipment list has been populated." });
    } catch (error) {
      console.error(error);
    ...
    <div className="flex flex-col gap-8 p-4 md:p-8 max-w-7xl mx-auto">
      <header className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-black tracking-tight text-slate-900 uppercase">ROASTER MCC 4</h1>
        ...
        <div className="flex items-center gap-3">
          ...
          <Link href={`/equipment/new?mcc=Roaster MCC 4&plant=Smelter&division=Roaster&location=Smelter`}>
      ...
      <Card className="border-slate-200 shadow-sm">
        <CardHeader className="bg-slate-50 border-b border-slate-100 pb-4">
          ...
          <CardDescription className="text-xs text-slate-500">
            Manage all variable speed drives, motors, and protection systems powered by Roaster MCC 4.
          </CardDescription>
        </CardHeader>
