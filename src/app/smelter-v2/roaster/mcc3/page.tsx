...
export default function RoasterMCC3Page() {
  ...
  const equipmentQuery = useMemoFirebase(
    () => query(collection(firestore, 'equipment'), where('mcc', '==', 'Roaster MCC 3')),
    [firestore]
  );
  ...
  const handleSeedData = async () => {
    ...
    const items = [
      { name: "Howden Blower 1", type: "VSD", manufacturer: "ABB", model: "ACS880" },
      { name: "Howden Blower 2", type: "VSD", manufacturer: "ABB", model: "ACS880" }
    ];
    for (let i = 1; i <= 8; i++) {
      items.push({ name: `RTR ${i}A (Feeder)`, type: "VSD", manufacturer: "Siemens", model: "Micromaster 440" });
      items.push({ name: `RTR ${i}B (Drum)`, type: "VSD", manufacturer: "Siemens", model: "Micromaster 440" });
    }
    items.push({ name: "Weigh Feeder", type: "VSD", manufacturer: "Siemens", model: "Micromaster 440" });
    items.push({ name: "Hammer Sampler", type: "VSD", manufacturer: "Siemens", model: "Micromaster 440" });
    ...
    try {
      ...
      toast({ title: "Success", description: "Roaster MCC 3 equipment list has been populated." });
    } catch (error) {
      console.error(error);
    ...
    <div className="flex flex-col gap-8 p-4 md:p-8 max-w-7xl mx-auto">
      <header className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          <Link href="/smelter-v2/roaster">
            <Button variant="ghost" size="icon" className="rounded-full hover:bg-slate-200">
              <ArrowLeft className="h-5 w-5" />
            </Button>
          </Link>
          <div>
            <h1 className="text-3xl font-black tracking-tight text-slate-900 uppercase">ROASTER MCC 3</h1>
        ...
        <div className="flex items-center gap-3">
          ...
          <Link href={`/equipment/new?mcc=Roaster MCC 3&plant=Smelter&division=Roaster&location=Smelter`}>
      ...
      <Card className="border-slate-200 shadow-sm">
        <CardHeader className="bg-slate-50 border-b border-slate-100 pb-4">
          ...
          <CardDescription className="text-xs text-slate-500">
            Manage all variable speed drives, motors, and protection systems powered by Roaster MCC 3.
          </CardDescription>
        </CardHeader>
