...
export default function CommonPage() {
  ...
  const equipmentQuery = useMemoFirebase(
    () => query(collection(firestore, 'equipment'), where('division', '==', 'Smelter - Common')),
    [firestore]
  );
  ...
  const handleSeedData = async () => {
    ...
    const items = [
      { name: "Conveyor west", type: "Soft Starter", manufacturer: "Allen Bradley", model: "SMC-FLEX", mcc: "Furnace 1 MCC" },
      { name: "Tripper Car East", type: "VSD", manufacturer: "Control Techniques", model: "UNISP", mcc: "Bunker Floor" },
      { name: "Tripper Car West", type: "VSD", manufacturer: "Control Techniques", model: "UNISP", mcc: "Bunker Floor" },
    ];
    ...
    try {
      ...
      toast({ title: "Success", description: "Smelter Common equipment list has been populated." });
    } catch (error) {
      console.error(error);
    ...
    <div className="flex flex-col gap-8 p-4 md:p-8 max-w-7xl mx-auto">
      <header className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-black tracking-tight text-slate-900 uppercase">Smelter - Common</h1>
        ...
        <div className="flex items-center gap-3">
          ...
          <Link href={`/equipment/new?plant=Smelter&division=Smelter - Common&location=Smelter`}>
      ...
      <Card className="border-slate-200 shadow-sm">
        <CardHeader className="bg-slate-50 border-b border-slate-100 pb-4">
          ...
          <CardDescription className="text-xs text-slate-500">
            Manage all variable speed drives, motors, and protection systems for the Smelter Common area.
          </CardDescription>
        </CardHeader>
