...
export default function ElevatorPage() {
  ...
  const equipmentQuery = useMemoFirebase(
    () => query(collection(firestore, 'equipment'), where('division', '==', 'Smelter Elevator')),
    [firestore]
  );
  ...
  const handleSeedData = async () => {
    ...
    const items = [
      { name: "Main Elevator Hoist Drive", type: "VSD", manufacturer: "OTIS", model: "N/A", mcc: "Otis Motor Room" },
    ];
    ...
    try {
      ...
      toast({ title: "Success", description: "Smelter Elevator equipment list has been populated." });
    } catch (error) {
      console.error(error);
    ...
    <div className="flex flex-col gap-8 p-4 md:p-8 max-w-7xl mx-auto">
      <header className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-black tracking-tight text-slate-900 uppercase">Smelter - Elevator</h1>
        ...
        <div className="flex items-center gap-3">
          ...
          <Link href={`/equipment/new?plant=Smelter&division=Smelter Elevator&location=Smelter`}>
      ...
      <Card className="border-slate-200 shadow-sm">
        <CardHeader className="bg-slate-50 border-b border-slate-100 pb-4">
          ...
          <CardDescription className="text-xs text-slate-500">
            Manage all variable speed drives, motors, and protection systems for the Smelter Elevator.
          </CardDescription>
        </CardHeader>
