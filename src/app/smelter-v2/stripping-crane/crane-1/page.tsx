...
export default function Crane1Page() {
  ...
  const equipmentQuery = useMemoFirebase(
    () => query(collection(firestore, 'equipment'), where('mcc', '==', 'Stripping Crane 1 MCC')),
    [firestore]
  );
  ...
  const handleSeedData = async () => {
    ...
    const items = [
      { name: "Main Hoist", type: "VSD", manufacturer: "Demag", model: "ACS800", mcc: "Stripping Crane 1 MCC" },
      { name: "Auxiliary hoist", type: "VSD", manufacturer: "Demag", model: "ACS800", mcc: "Stripping Crane 1 MCC" },
      { name: "Long travel", type: "VSD", manufacturer: "Demag", model: "ACS800", mcc: "Stripping Crane 1 MCC" },
      { name: "Cross travel", type: "VSD", manufacturer: "Demag", model: "ACS800", mcc: "Stripping Crane 1 MCC" },
    ];
    ...
    try {
      for (const item of items) {
        ...
        const eqData: any = {
          id: eqId,
          name: item.name,
          location: 'Stripping Cranes',
          plant: 'Smelter',
          division: 'Stripping Cranes',
          mcc: item.mcc, 
          vsdId: vsdId,
          assignedToName: '',
      ...
      toast({ title: "Success", description: "Stripping Crane 1 equipment list has been populated." });
    } catch (error) {
      console.error(error);
    ...
    <div className="flex flex-col gap-8 p-4 md:p-8 max-w-7xl mx-auto">
      <header className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          <Link href="/smelter-v2/stripping-crane">
            <Button variant="ghost" size="icon" className="rounded-full hover:bg-slate-200">
              <ArrowLeft className="h-5 w-5" />
            </Button>
          </Link>
          <div>
            <h1 className="text-3xl font-black tracking-tight text-slate-900 uppercase">Stripping Crane 1</h1>
        ...
        <div className="flex items-center gap-3">
          ...
          <Link href={`/equipment/new?plant=Smelter&division=Stripping Cranes&mcc=Stripping Crane 1 MCC&location=Stripping Cranes`}>
      ...
      <Card className="border-slate-200 shadow-sm">
        <CardHeader className="bg-slate-50 border-b border-slate-100 pb-4">
          ...
          <CardDescription className="text-xs text-slate-500">
            Manage all variable speed drives, motors, and protection systems for Stripping Crane 1.
          </CardDescription>
        </CardHeader>
