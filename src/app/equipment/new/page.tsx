...
import { NameplateScanner } from '@/components/equipment/nameplate-scanner';
...
function EquipmentFormContent() {
  ...
  useEffect(() => {
    if (searchParams) {
      const plantParam = searchParams.get('plant') as any;
      const divParam = searchParams.get('division') as any;
      const mccParam = searchParams.get('mcc') as any;
      const locParam = searchParams.get('location') as any;
      
      if (plantParam) form.setValue('plant', plantParam);
      if (divParam) form.setValue('division', divParam);
      if (mccParam) form.setValue('mcc', mccParam);
      if (locParam) form.setValue('location', locParam);
    }
  }, [searchParams, form]);

  const handleAiScanResult = (data: any) => {
    // Fill technical fields from AI extraction
    if (data.model) form.setValue('model', data.model);
    if (data.serialNumber) form.setValue('serialNumber', data.serialNumber);
    if (data.motorModel) form.setValue('motorModel', data.motorModel || data.model);
    if (data.serialNumber) form.setValue('motorSerialNumber', data.serialNumber);
    if (data.motorPower) form.setValue('motorPower', data.motorPower);
    if (data.motorVoltage) form.setValue('motorVoltage', data.motorVoltage);
    if (data.motorFrameType) form.setValue('motorFrameType', data.motorFrameType);
    
    // Attempt to build IDs if empty
    if (!form.getValues('equipmentId') && data.model && data.serialNumber) {
      const slug = `${data.model}-${data.serialNumber}`.toLowerCase().replace(/[^a-z0-9]/g, '-');
      form.setValue('equipmentId', slug);
      form.setValue('vsdId', `vsd-${slug}`);
    }
  };
  ...
  function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    const equipmentData: Partial<Equipment> = {
      ...
      upsType: values.equipmentProfile === 'UPS/BTU' ? 'UPS' : undefined,
    ...
    // Strip any undefined fields to prevent Firebase errors
    Object.keys(equipmentData).forEach(key => {
      if ((equipmentData as any)[key] === undefined) {
        delete (equipmentData as any)[key];
      }
    });
    ...
    // Go back if there's a history, otherwise reset
    if (searchParams?.has('mcc')) {
      router.back();
    } else {
      form.reset();
    }
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      {/* AI Nameplate Scanner Integration */}
      <Card className="border-primary/20 bg-primary/5">
        <CardHeader>
          <CardTitle className="text-sm uppercase tracking-wider text-primary font-bold flex items-center gap-2">
            <Sparkles className="h-4 w-4" /> Fast Track Entry
          </CardTitle>
          <CardDescription>Use AI to scan technical details from a nameplate photo.</CardDescription>
        </CardHeader>
        <CardContent>
          <NameplateScanner onDataExtracted={handleAiScanResult} />
        </CardContent>
      </Card>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem className="md:col-span-2">
                    <FormLabel>Equipment Profile</FormLabel>
                    <Select onValueChange={field.onChange} defaultValue={field.value}>
                      ...
                      <SelectContent>
                        <SelectItem value="Standard">Standard Motor/Pump System</SelectItem>
                        <SelectItem value="UPS/BTU">UPS/BTU System</SelectItem>
                      </SelectContent>
                    </Select>
                    <FormMessage />
                  </FormItem>
