...
const btuData = [
  {
    name: "BTU's (Silicon, Electrical Manufacturing, Blue Ginger - 10amp)",
    amount: 27,
    assignedTo: 'N/A',
...
const upsData = [
  {
    name: "UPS's (Riello HPS, MPM, MPS - 10Kva to 80Kva)",
    amount: 47,
    assignedTo: 'N/A',
    breakdownStatus: 'None',
    uptime: '100%',
...
export default function SmelterUpsBtusPage() {
  const { user } = useUser();
  ...
  return (
    <div className="flex flex-col gap-8">
      <header className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">Equipment: Smelter Feed and Products (SP&L)</h1>
          <p className="text-muted-foreground">
            All monitored UPS and BTU units in the Smelter plant.
          </p>
        </div>
        {!isClientManager && (
      ...
      <Accordion type="multiple" className="w-full" defaultValue={['item-btus', 'item-ups']}>
        <AccordionItem value="item-btus">
          <AccordionTrigger className="text-xl font-semibold">
            BTU's ({btuData.reduce((sum, item) => sum + item.amount, 0)})
          </AccordionTrigger>
          <AccordionContent>
            <Card>
        ...
        <AccordionItem value="item-ups">
          <AccordionTrigger className="text-xl font-semibold">
            UPS's ({upsData.reduce((sum, item) => sum + item.amount, 0)})
          </AccordionTrigger>
          <AccordionContent>
            <Card>
              <CardContent className="pt-6">
